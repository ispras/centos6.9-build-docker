...
RUN curl -L -O https://github.com/Kitware/CMake/releases/download/v3.21.3/cmake-3.21.3-linux-x86_64.sh && \
    sh cmake-3.21.3-linux-x86_64.sh --prefix=/ --exclude-subdir --skip-license && \
...
RUN curl -O https://ftp.gnu.org/gnu/gcc/gcc-9.3.0/gcc-9.3.0.tar.xz && \
...
RUN curl -L -O https://github.com/ninja-build/ninja/archive/refs/tags/v1.10.2.zip && \
    unzip v1.10.2.zip && rm v1.10.2.zip && \
    cd ninja-1.10.2 && mkdir build && cd build && \
    cmake .. && cmake --build . -j200 && cp ninja /usr/bin && \
    cd ../.. && rm -rf ninja-1.10.2

RUN curl -O https://www.python.org/ftp/python/2.7.18/Python-2.7.18.tar.xz && \
...
RUN curl -L -O https://github.com/llvm/llvm-project/releases/download/llvmorg-10.0.0/llvm-project-10.0.0.tar.xz && \
